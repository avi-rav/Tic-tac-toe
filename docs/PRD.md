...
- **NFR-4 (Testability):** pure logic unit-tested; key UI flows covered by React Testing Library.

## 6. Architecture Summary

Three layers keep responsibilities separate (see `docs/HLD.md` for the full design):
1. **Pure rules** (`src/game/`) — `calculateWinner`, `isDraw`, `getNextPlayer`; data-driven by `WINNING_LINES` (Open/Closed).
2. **State** (`src/hooks/useGame.ts`) — the single owner of mutable game state; guards illegal moves.
3. **Presentation** (`src/components/`) — dumb, focused components that depend on the hook abstraction, not on the rules directly (Dependency Inversion).
...
```
App                              owns: players | null  (setup ↔ play seam)
│
...
└── Game                         shown when players !== null; calls useGame()
      ...
      └── GameControls           New Game · Reset Scores · Change Players
```
...
|---|---|---|---|---|
| `App` | Stateful (seam) | Hold session players; swap setup ↔ game | — | — |
| `PlayerSetup` | Form (RHF) | Collect & validate two distinct names | `onStart(players)` | `onStart` on valid submit |
| `Game` | Composition root | Pull state from `useGame`, wire to children | `players`, `onChangePlayers` | delegates to hook actions |
...
| `Board` | Presentational | Map board array → 9 Squares; flag winning line | `board`, `winningLine`, `disabled`, `onCellClick` | `onCellClick(index)` |
...
| `GameControls` | Presentational | Emit New Game / Reset Scores / Change Players intents | `onNewGame`, `onResetScores`, `onChangePlayers` | the three callbacks |

### 6a.3 SOLID mapping

- **SRP:** rules (`gameLogic`), state (`useGame`), and presentation (components) never mix; validation lives only in `playerSchema`.
- **OCP:** win detection and turn order are driven by `WINNING_LINES` / `PLAYERS` data — extendable without editing logic.
- **LSP:** every `Square` is substitutable; it carries no game knowledge.
- **ISP:** each component receives only the props it renders (e.g. `Square` takes a value + flags, not the whole board).
- **DIP:** presentation depends on the `UseGameResult` abstraction and callbacks, not on the pure rules directly.

## 7. Acceptance Criteria

All functional requirements above are demonstrable in the running app, and the documented **UI test cases** (`TEST_CASES.md`) pass — both the written scenarios and the implemented automated subset (`npm run test`).
